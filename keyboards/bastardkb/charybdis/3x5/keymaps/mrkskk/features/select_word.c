...
#    endif                                      // NO_ACTION_ONESHOT
        if ((all_mods & MOD_MASK_SHIFT) == 0) { // Select word.
            register_code(is_windows() ? KC_LCTL : KC_LALT);
            if (state == STATE_NONE) {
...
#    ifndef NO_ACTION_ONESHOT
                clear_oneshot_mods();
#    endif // NO_ACTION_ONESHOT
                send_string((is_windows()) ? SS_TAP(X_HOME) SS_LSFT(SS_TAP(X_END)) : SS_LCTL("a" SS_LSFT("e")));
                set_mods(mods);
    ...
    switch (state) {
        case STATE_WORD:
            ...
            unregister_code(is_windows() ? KC_LCTL : KC_LALT);
            // tap_code16(is_windows() ? C(KC_C) : G(KC_C));
